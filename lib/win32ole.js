...
/*
  ...
  VT_BSTR (VT_CLSID, VT_FILETIME, VT_LPWSTR, VT_LPSTR,
    VT_DECIMAL, VT_ERROR, VT_BSTR, VT_DATE) <-> string <-> Utf8Value
...
var mm = require('./metamorphoses');

V8Variant.prototype.call = function(){
  var a = arguments.length < 2 ? [] : arguments[1];
//  return mm.metamorphoses(this.OLECall(arguments[0], a));
  return this.OLECall(arguments[0], a);
};

V8Variant.prototype.get = function(methodName, args){
  var a = arguments.length < 2 ? [] : arguments[1];
//  return mm.metamorphoses(this.OLEGet(arguments[0], a));
  return this.OLEGet(arguments[0], a);
};

V8Variant.prototype.set = function(methodName, args){
  var a = arguments.length < 2 ? [] : arguments[1];
  return this.OLESet(arguments[0], a);
};

// accessors

V8Variant.prototype.__noSuchMethod__ = function(methodName, args){
  // console.log('method: ' + methodName + '(' + args + ')');
  return this.call(methodName, args);
};

V8Variant.prototype.__noSuchGetter__ = function(getterName, args){
  // console.log('getter: ' + getterName + '(' + args + ')');
  return this.get(getterName, args);
};

V8Variant.prototype.__noSuchSetter__ = function(setterName, args){
  // console.log('setter: ' + setterName + ' = ' + args);
  return this.set(setterName, args);
};
...
win32ole.verbose = function(){
  if(!isVerbose){
    ...
    trace.extendTrace(V8Variant.prototype, 'toInt64');
    trace.extendTrace(V8Variant.prototype, 'toNumber');
    trace.extendTrace(V8Variant.prototype, 'toUtf8');
    trace.extendTrace(V8Variant.prototype, 'OLECall');
    trace.extendTrace(V8Variant.prototype, 'OLEGet');
    trace.extendTrace(V8Variant.prototype, 'OLESet');
...
//    trace.extendTrace(V8Variant.prototype, '__');
//    trace.extendTrace(V8Variant.prototype, '_');
...
/**/
win32ole.client.Dispatch = function(args){
  return mm.metamorphoses(win32ole.Client.prototype.Dispatch.call(this, args));
};
/**/
